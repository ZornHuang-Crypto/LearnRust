...
/// A naive htttpie implementation with Rust, can you imagine how easy it is?
#[derive(Clap, Debug)]
#[clap(version = "1.0", author = "CR Hunag <CR@crios.cn>")]
...
// 子命令分别对应不同的HTTP 方法，目前只支持get / post
#[derive(Clap, Debug)]
enum SubCommand {
    Get(Get),
    Post(Post)
...
/// 命令行重的key = value 可以通过parse_kv_pair 解析成KvPair结构
