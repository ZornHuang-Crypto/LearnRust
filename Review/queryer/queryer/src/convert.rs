...
use sqlparser::ast::{
    ...
    Offset as SqlOffset, OrderByExpr, Select,
...
// 因为Rust trait的孤儿规则，我们如果要想对已有的类型实现已有的trait，
// 需要简单包装一下
...
/// 把SqlParser 解析出来的statement转换成我们需要的结构
impl <'a> TryFrom<&'a Statement> for Sql<'a> {
    type Error = anyhow::Error;

    fn try_from(sql: &'a Statement) -> Result<Self, Self::Error> {
        match sql {
            // 目前我们只关心query (select ... from ... where ...)
            Statement::Query(q) => {
...
/// 把SqlParser 的 Expr转换成DataFrame 的 Expr
...
/// 把SqlParser的BinaryOperator 转换成DataFrame的Operator
...
/// 把SqlParser 的value转换成DataFrame支持的LiteralValue
...
mod tests {
    ...
    fn parse_sql_works() {
        ...
        let statement = &Parser::parse_sql(&CRDialect::default(), sql.as_ref()).unwrap()[0];
